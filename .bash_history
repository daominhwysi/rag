pip install -r requirements.txt
pip install -r requirements.txt
pip install -r requirements.txt
pip install -r requirements.txt
cd rag
cd ..
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
/home/zeus/miniconda3/envs/cloudspace/bin/python
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python -m src.provider.embedding.siliconflow 
python -m src.provider.embedding.siliconflow 
python -m src.provider.embedding.siliconflow 
python -m src.provider.embedding.siliconflow 
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python scripts/build_index.py
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
pip install -r requirements.txt
python -m scripts.build_index
python -m scripts.build_index
python -m scripts.build_index
docker run   -p 6333:6333 \                  # mở cổng 6333 ra ngoài
  -v $(pwd)/qdrant_storage:/qdrant/storage \  # map thư mục lưu trữ ra ổ đĩa thật
  qdrant/qdrant                   # image qdrant
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.debug_qdrant
python -m scripts.debug_qdrant
python -m scripts.push_qdrant
docker run -p 6333:6333 -v $(pwd)/qdrant_storage:/qdrant/storage qdrant/qdrant
python -m scripts.push_qdrant
python -m scripts.debug_qdrant
python -m scripts.debug_qdrant
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.search
pip install -r requirements.txt
pip install -r requirements.txt
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.push_qdrant
docker run -p 6333:6333 -v $(pwd)/qdrant_storage:/qdrant/storage qdrant/qdrant
rm -rf qdrant_storage
sudo rm -rf qdrant_storage
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.search
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.search
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.search
docker run -p 6333:6333 -v $(pwd)/qdrant_storage:/qdrant/storage qdrant/qdrant
python -m scripts.push_qdrant
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
python -m scripts.search
docker run -p 6333:6333 -v $(pwd)/qdrant_storage:/qdrant/storage qdrant/qdrant
git init .
git remote add origin https://github.com/daominhwysi/rag
git add .
git add .
GEMINI_API_KEY=AIzaSyCGLBF_aUfgAecNm5kVg3jMyYzsvgiMJRk
SILICONFLOW_API_KEY=sk-pnnvxapbhfrpztchndhbrhpcgyplzhjbjlajzlagbquvubuo
git commit -m "initial commit"
git push -u origin main
gh auth login
gh auth login
gh auth login
git push -u origin main
git commit -m "initial commit"
